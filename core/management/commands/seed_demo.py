"""
Seed all the "operational" datasets for the Saint Club Limited backend --
choice/lookup tables, attendance, restaurant, outlets, reservations,
payroll, vendors, finance, and events -- WITHOUT touching accounts.

This deliberately does NOT create any CustomUser, Member, or StaffProfile
rows. It assumes those already exist, created by the real account
pipeline:

    python manage.py migrate
    python manage.py bootstrap_auth
    python manage.py bootstrap_admin
    python manage.py seed_accounts --members 50 --staff 50 --pending 10
    python manage.py seed_data

If no superuser or no Members/StaffProfiles are found, this command
fails fast with a clear message instead of silently creating fallback
accounts (unlike seed_demo.py / seed_huge.py, which both create a
fallback "admin" user themselves) -- account creation is intentionally
out of scope here, that's what seed_accounts / bootstrap_admin are for.

Built from the proven, schema-correct logic in seed_huge.py (see that
file's docstring), stripped of everything account/member/staff-related
and made standalone (no inheritance from seed_demo.Command, so this
doesn't depend on the seed_staff_permissions command referenced there,
which doesn't exist in this codebase).

Usage:
    python manage.py seed_data                    # big defaults
    python manage.py seed_data --months 12
    python manage.py seed_data --flush             # wipe previously
                                                    # seeded operational
                                                    # data first (leaves
                                                    # accounts/members
                                                    # untouched)
...
FIRST_NAMES = ["Arif", "Karim", "Rahim", "Sadia", "Nadia", "Tanvir", "Fahim",
               ...
               "Adib", "Bushra", "Chayan", "Dalia", "Emon", "Farhana", "Galib",
...
LAST_NAMES = ["Hossain", "Islam", "Ahmed", "Khan", "Chowdhury", "Rahman",
              "Akter", "Uddin", "Haque", "Bhuiyan", "Mridha", "Sarkar",
              "Talukder", "Mollah", "Sheikh", "Patwary", "Majumder",
              "Biswas", "Sikder", "Pramanik", "Howlader", "Joarder"]

BLOOD = ["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"]


class Command(BaseCommand):
    help = ("Seed operational data (attendance, restaurant, outlets, "
            "reservations, payroll, vendors, finance, events) against "
            "EXISTING members/staff/admin. Does not create any accounts.")

    def add_arguments(self, parser):
        parser.add_argument("--months", type=int, default=6,
                             help="How many months of historical activity")
        parser.add_argument("--flush", action="store_true",
                             help="Delete previously seeded operational "
                                  "data first (accounts/members untouched)")
    ...
    def handle(self, *args, **opts):
        ...
        if not self.admin:
            raise CommandError(
                "No superuser found. Run `python manage.py bootstrap_admin` "
                "first -- this command does not create accounts."
            )

        self.staff_profiles = list(StaffProfile.objects.all())
        if not self.staff_profiles:
            raise CommandError(
                "No StaffProfile records found. Run `python manage.py "
                "seed_accounts --staff 50` first -- this command does not "
                "create accounts."
            )
        ...
        if not members:
            raise CommandError(
                "No Member records found. Run `python manage.py "
                "seed_accounts --members 50` first -- this command does "
                "not create accounts."
            )

        self.stdout.write(self.style.WARNING(
            f"Seeding operational data against {len(members)} existing "
            f"members and {len(self.staff_profiles)} existing staff "
            f"profiles, {self.months} months of history..."))
        ...
        self.stdout.write(self.style.SUCCESS("Operational data seed complete."))
        self._summary()

    # ----------------------------------------------------------------
    def _flush(self):
        """
        Deletes previously seeded *operational/transactional* rows only.
        Deliberately does not touch CustomUser, Member, or StaffProfile --
        those belong to the account pipeline, not this command.
        """
        from attendance.models import AttendanceRecord, Guest, RFIDCard
        from restaurant.models import RestaurantOrder
        from outlet.models import OutletOrder
        from reservation.models import Reservation
        from payroll.models import Payslip, PayrollRun, SalaryStructure, StaffLoan
        from vendor.models import VendorServiceOffer, VendorPayment
        from finance_core.models import Expense
        from event.models import Event

        self.stdout.write("  flushing previously seeded operational rows...")
        for M in [AttendanceRecord, Guest, RFIDCard, RestaurantOrder,
                  OutletOrder, Reservation, Payslip, PayrollRun, StaffLoan,
                  SalaryStructure, VendorPayment, VendorServiceOffer,
                  Expense, Event]:
    ...
    def _seed_choice_models(self):
        ...
        self.genders = [safe_get_or_create(Gender, n)
                         for n in ["Male", "Female", "Other"]]
        self.mtypes = [safe_get_or_create(MembershipType, n)
                        for n in ["General", "Life", "Corporate", "Honorary"]]
        self.institutes = [
            safe_get_or_create(InstituteName, n, {"code": f"INST{idx:02d}"})
            for idx, n in enumerate(
                ["Dhaka University", "BUET", "NSU", "BRAC"], start=1)
        ]
        self.mstatus = [safe_get_or_create(MembershipStatusChoice, n)
                         for n in ["Active", "Suspended", "Pending"]]
        self.marital = [safe_get_or_create(MaritalStatusChoice, n)
                         for n in ["Single", "Married"]]
        self.stdout.write(
            f"  choice models: {len(self.genders)} genders, "
            f"{len(self.mtypes)} membership types, {len(self.institutes)} "
            f"institutes, {len(self.mstatus)} membership statuses, "
            f"{len(self.marital)} marital statuses")
    ...
    def _seed_attendance(self, members):
        ...
        for idx, sp in enumerate(self.staff_profiles):
        ...
        if not cards:
            self.stdout.write("  attendance: no member cards to attach "
                               "records to, skipping records/guests")
            return

        rec_target = len(carded) * self.months
        made = 0
        now = timezone.now()
        for _ in range(rec_target):
    ...
    def _summary(self):
        ...
        rows = [
            ("Members (pre-existing)", Member.objects.count()),
            ("Staff profiles (pre-existing)", len(self.staff_profiles)),
